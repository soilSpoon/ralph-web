...
import { formatPatternCategory, formatDate } from '@/lib/formatters';
      ...
      <Card>
        <CardContent className="pt-6">
          <div className="flex items-center justify-between mb-4">
            ...
            <span className="text-sm text-muted-foreground">{mockPatterns.length} patterns total</span>
          </div>
          <div className="flex flex-wrap gap-2">
            {categories.map((category) => (
              <Badge key={category.value} variant="outline" className="cursor-pointer hover:bg-accent">
              ...
              <div className="flex items-start gap-3">
                ...
                <div className="flex-1 space-y-2">
                  <div className="flex items-start justify-between gap-2">
                    ...
                    <Badge variant="secondary">{formatPatternCategory(pattern.category)}</Badge>
                  </div>
                  {pattern.sourceFile && (
                    <p className="text-sm text-muted-foreground">
                      Source: <code className="bg-muted px-1 py-0.5 rounded text-xs">{pattern.sourceFile}</code>
                    </p>
                  )}
                  <p className="text-caption text-muted-foreground">
                    Created: {formatDate(pattern.createdAt)}
                    {pattern.promotedAt && ` • Promoted: ${formatDate(pattern.promotedAt)}`}
                  </p>
                </div>
              </div>
            </CardContent>
